...
router.post('/modify/:articleID',function (req,res,next) {
  ...
  var query = 'UPDATE article SET articleTitle=' + mysql.escape(title)+',articleContent='+mysql.escape(content) +"WHERE articleId="+
...
/*the information of user to check */
router.post('/login',function(req,res,next){
    //console.log(req.body.name);
    
    var name=req.body.name;
    var password=req.body.password;
    var hash=crytro.createHash('md5');
    hash.update(password);
    password=hash.digest('hex');
    console.log(mysql.escape(password))
    ...
    mysql.query(query,function(err,rows,fields){
      ...
      //进入路由时还没有将express-session与app绑定。
    ...
    //console.log(req.session.user+'user')
      // req.session.userSign=true;   undefined....how to my yor seesion...
  ...
  var query = 'SELECT * FROM article WHERE articleID=' + mysql.escape(articleID);
    ...
    mysql.query(query,function(err,rows,fields){
        if(err) {
         ...
         res.render('article', {article:article,user:req.session.user});/*,user:req.session.user}*/
  });
});
router.get('/', function(req, res, next) {
    var query = 'SELECT * FROM article ORDER BY articleID DESC';
